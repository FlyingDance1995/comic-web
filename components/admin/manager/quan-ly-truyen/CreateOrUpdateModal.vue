...
const getTeam = async () => {
    try {
        const response = await useNuxtApp().$api('/admin/teams', {
            method: "GET",
            query: {
                size: 1000,
            }
    ...
    formRef.value.validate(async valid => {
        if (valid) {
            try {
                ...
                await useNuxtApp().$api(dataEdit.value ? `/admin/story/${dataEdit?.value?.slug}` : '/admin/story', {
            ...
            <Row :gutter="20">
                <Col flex="200px">
                    <div class="avatar-wrapper mb-3">
                        ...
                        <input ref="uploadRef" class="file-upload" type="file" name="avatar"
